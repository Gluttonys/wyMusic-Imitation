...
export default {
  ...
  mounted() {
    ...
    eventBus.$on('getMusic', mID => {
      ...
      getInfoMusic(mID)
          .then(data => this.collectionData(data, mID))
          .catch(error => inError.call(this, '歌曲信息', error))
    })
  },
  methods: {
    collectionData(data, mId) {
      // 检测是否时付费歌曲
  ...
  watch: {
    'musicInfo.mId': function (newValue, _) {
      if (/\/music\/\d+/.test(this.$route.fullPath)) {
        this.$router.push(`/music/${newValue}`)
      }
    }
  }
}
